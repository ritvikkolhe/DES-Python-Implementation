...
def encrypt():
    ...
    file_write=open(sys.argv[3],'wb')
    counter=0
    while counter<3:
        bitvec = bv.read_bits_from_file(8)
        char = bitvec.get_bitvector_in_ascii()
        if char=="\n":
            counter+=1
        bitvec.write_to_file(file_write)
    while (bv.more_to_read):
        ...
        for round in range(16):
            ...
            out_xor = newRE ^  round_key[round]
            RE_modified = substitute(out_xor)
            right_half = RE_modified.permute( p_box )
            temp = RE
            RE = right_half ^ LE
            LE = temp
        final_string = RE+LE
        text_bv+=final_string
    text_bv.write_to_file(file_write)
